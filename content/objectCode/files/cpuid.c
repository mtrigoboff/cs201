...
// test for presence of CPUID instruction by attempting to change bit 21 of EFLAGS as described
// on page 7 of "Intel Processor Identification and the CPUID Instruction", published by Intel
//
int cpuidAvailable ()
{
    int     eflags1, eflags2;
...
int main ()
{
    ...
    cpuid(0, &max, (int *) vendorStr, (int *) &vendorStr[8], (int *) &vendorStr[4]);
    vendorStr[12] = '\0';
    printf("%-*s %d\n", hdrWidth, "max EAX value, normal range:", max);
    printf("%-*s \"%s\"\n\n", hdrWidth, "vendor string:", vendorStr);

    cpuid(0x80000000, &extMax, 0, 0, 0);
    printf("%-*s %d(0x%08X)\n", hdrWidth, "max EAX value, extended range:", extMax & 0x7FFFFFFF, extMax);
    if (extMax >= 0x80000004) {
        ...
        printf("%-*s \"%s\"\n", hdrWidth, "brand string:", (char *) brandStr);
        }
